...
namespace St4mpede.Poco
{
	...
	internal class PocoGenerator
	{
		...
		private IList<string> MakeClass(ClassData classData)
		{
			...
			var classCode = new St4mpede.Code.ClassData
			{
				Name = classData.Name,
				Properties = classData.Properties.Select(p =>
				   new St4mpede.Code.PropertyData
				   {
						Name = p.Name,
						SystemType = Type.GetType(p.DotnetTypeName),
						Scope = St4mpede.Code.Common.VisibilityScope.Public // TODO:Fix.
					}
				).ToList()
			};

			ret.AddRange(classCode.ToCode());
